...
import type { CreateMovieDto, EditMovieDto } from './dto'
...
export class MovieController {
  ...
  @Get(':id')
  getMovieById(@GetUser('id') userId: number, @Param('id', ParseIntPipe) movieId: number) {
  ...
  @Patch(':id')
  editMovie(@GetUser('id') userId: number, @Param('id', ParseIntPipe) movieId: number, @Body() dto: EditMovieDto) {
    return this.movieService.editMovie(userId, movieId, dto)
  }

  @HttpCode(HttpStatus.NO_CONTENT)
  @Delete(':id')
  deleteMovie(@GetUser('id') userId: number, @Param('id', ParseIntPipe) movieId: number) {
