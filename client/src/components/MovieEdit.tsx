...
import type { MovieData, ServerErrorResponse } from '../interfaces'
...
export default function MovieEdit() {
  const { title, overview, homepage, imgUrl, genre } = useLoaderData() as MovieData
  ...
  return (
    <div className='flow container'>
      <Link to='..' relative='path' state={{ pathname, search, genre_type }}>
        &larr; <span>Back </span>
      </Link>
      <div className='flow container grid grid-items-center width-300'>
        <Form className='grid' replace method='patch'>
          <input name='title' type='text' placeholder='Title' defaultValue={title} />
          <input name='homepage' type='text' placeholder='Homepage Url' defaultValue={homepage} />
          <input name='imgUrl' type='text' placeholder='Image Url' defaultValue={imgUrl} />
          <textarea name='overview' placeholder='Description' defaultValue={overview} />
