...
export default function Movies() {
  ...
  function renderMovies(movies: MovieData[]) {
    const genreFilteredMovies = genreFilter ? movies.filter((movie) => movie.genre === genreFilter) : movies
    ...
    const moviesElements = displayedMovies.map(({ id, title, imgUrl, genre }, i) => (
      <Link
    ...
    return (
      <>
        <Link
          className='flex flex-right text-green'
          to='../create'
        ...
        <div className='container flex flex-space-between'>
          <div className='flex'>
            <button className='bg-blue text-white' onClick={() => handleFilterChange('sort_title', 'up')}>
              &#x2191;
            </button>
            <button className='bg-blue text-white' onClick={() => handleFilterChange('sort_title', 'down')}>
              &#x2193;
            </button>
          </div>
  ...
  return (
    <div className='flow container movies-list-container'>
      ...
      <Suspense fallback={<h2>Loading movies...</h2>}>
        <Await resolve={moviesPromise.movies}>{renderMovies}</Await>
      </Suspense>
    </div>
  )
}
